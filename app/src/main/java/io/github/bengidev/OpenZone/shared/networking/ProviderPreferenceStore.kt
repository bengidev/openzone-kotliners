...
/**
 * Persists the user's non-secret provider/model selection across launches.
 * Feature-neutral seam in `shared/networking/`: Settings writes it, Home/Chat
 * read it to resolve which provider+model a request targets. No secrets here —
 * the API key lives in [MutableCredentialStore].
 *
...
interface ProviderPreferenceStore {
    ...
    /** Persists the selected provider id, preserving any existing model. */
