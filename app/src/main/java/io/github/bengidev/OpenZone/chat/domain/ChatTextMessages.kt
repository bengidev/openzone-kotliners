...
/**
 * Default static user-facing text used by the mock client.
 * Kept in `domain` so any feature (tests, previews, future real client) can reuse it
 * without depending on Compose or the `infrastructure` package.
 * Mirrors iOS `ChatTextMessages`.
 */
object ChatTextMessages {
    /** Greeting shown when the user has not typed anything yet. */
    const val EMPTY_GREETING: String =
    ...
    val REPLY_TEMPLATES: List<(String) -> String> = listOf(
        { trimmed ->
            """
            ...
            When a real API integration is ready, the mock streaming client can simply be \
        ...
        { trimmed ->
            ...
            Note that this architecture follows a unidirectional state pattern: each incoming \
            delta is merged into the existing message row by a stable ID, rather than \
        ...
        { trimmed ->
            ...
            Technical detail: the inter-token delay can be tuned via `delayMillis` and \
            `thinkingDelayMillis`. The defaults give a comfortable typing feel, while \
    ...
    val THINKING_STEPS: List<String> = listOf(
        "Reading the user message: \"%s\". ",
    ...
    const val THINKING_TAIL: String =
        "Reviewing the answer once more to ensure consistency. " +
            "Tidying up the final points and adding a short summary. Done."

    const val THINKING_EMPTY: String =
        "Setting up the initial context. No user input yet, " +
