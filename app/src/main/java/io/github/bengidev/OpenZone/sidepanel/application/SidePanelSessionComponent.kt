 ...
 *
 * Sidebar presentation (open/close) is owned by the parent
 * [io.github.bengidev.openzone.home.application.HomeComponent]; this component owns the
 * conversation list, search query, and active-conversation highlight.
 */
class SidePanelSessionComponent(
   ...
   fun onRenameConversation(conversation: ChatConversation, newTitle: String) {
      ...
      scope.launch {
         withContext(ioDispatcher) { historyStore.renameConversation(conversation.id, title) }
         reloadConversations()
   ...
   fun onDeleteConversation(conversation: ChatConversation) {
      scope.launch {
         withContext(ioDispatcher) { historyStore.deleteConversation(conversation.id) }
         reloadConversations()
